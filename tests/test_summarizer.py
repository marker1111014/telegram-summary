...
async def test_generate_summary_happy_path(patch_generate):
    patch_generate(lambda prompt, generation_config=None, **kwargs: FakeResponse("*Summary* here"))
...
async def test_generate_summary_blocked_when_no_candidates(patch_generate, monkeypatch):
    ...
    patch_generate(lambda prompt, generation_config=None, **kwargs: blocked)
...
async def test_generate_summary_safety_finish_reason(patch_generate):
    ...
    resp = FakeResponse("")
    resp.candidates = [SimpleNamespace(finish_reason=Candidate.FinishReason.SAFETY)]
    patch_generate(lambda prompt, generation_config=None, **kwargs: resp)
...
async def test_generate_summary_timeout(patch_generate, monkeypatch):
    ...
    def slow(prompt, generation_config=None, **kwargs):
        import time
        time.sleep(1)
...
async def test_generate_summary_api_error(patch_generate):
    ...
    def boom(prompt, generation_config=None, **kwargs):
...
@pytest.mark.asyncio
async def test_generate_summary_safety_enum_not_string(patch_generate):
    """Regression: proto IntEnum finish_reason must match by .name, not =="SAFETY"."""
    from google.ai.generativelanguage_v1beta.types import Candidate

    class NoTextResponse:
        text = property(lambda self: (_ for _ in ()).throw(ValueError("safety stop has no parts")))
        candidates = [SimpleNamespace(finish_reason=Candidate.FinishReason.SAFETY)]
